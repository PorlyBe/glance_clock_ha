...
from .const import GLANCE_SERVICE_UUID, GLANCE_CHARACTERISTIC_UUID
...
def parse_color_input(color_input, default_color):
    ...
    if isinstance(color_input, list) and len(color_input) >= 3:
        try:
            ...
            _LOGGER.warning(f"Invalid RGB array format: {color_input}, using default")
            return default_color
    ...
    if isinstance(color_input, str):
        ...
        try:
            ...
            _LOGGER.warning(f"Invalid color format: {color_input}, using default")
            return default_color
    
    _LOGGER.warning(f"Unsupported color type: {type(color_input)}, using default")
    return default_color
...
def interpolate_color(value, min_val, max_val, min_color, max_color):
    ...
    g = int(min_g + (max_g - min_g) * factor)  # Fixed: was using min_b instead of min_g
...
DOMAIN = "glance_clock"
PLATFORMS = [Platform.NOTIFY, Platform.SENSOR, Platform.SWITCH, Platform.LIGHT, Platform.SELECT]
...
class GlanceClockConnectionManager:
    ...
    async def _notify_connection_callbacks(self):
        ...
        _LOGGER.debug(f"Notifying {len(self._connection_callbacks)} connection callbacks")
            ...
            try:
                ...
                if self.client and self.client.is_connected:
                    try:
                        ...
                        _LOGGER.warning(f"Connection ping failed for {self.name}: {e}")
    ...
    async def _connect(self):
        ...
        try:
            ...
            _LOGGER.error(f"Connection failed (attempt {self._reconnect_attempts}): {e}")

            if self._reconnect_attempts < self._max_reconnect_attempts:
                wait_time = 30 * self._reconnect_attempts
                await asyncio.sleep(wait_time)
            else:
                _LOGGER.error(f"Max reconnection attempts reached for {self.name}")
    ...
    async def send_command(self, command_data: bytes) -> bool:
        ...
        if not self.client or not self.client.is_connected:
            _LOGGER.warning(f"No active connection to {self.name}, attempting to connect")
            await self._connect()
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    _LOGGER.info(f"Setting up Glance Clock integration for {name} ({mac_address})")
        ...
        if notify_service:
            ...
            try:
                ...
                forecast_data = await hass.services.async_call(
                    ...
                    {"entity_id": weather_entity, "type": "hourly"},  # Changed to hourly
                    blocking=True,
                ...
                _LOGGER.debug(f"Looking for forecast at or after: {current_hour}")
                ...
                for i, hour in enumerate(forecast):
                    ...
                    if dt_str:
                        try:
                            ...
                            dt_local_hour = dt_local.replace(minute=0, second=0, microsecond=0)
                            
                            if first_forecast_time is None:
                                first_forecast_time = dt_local
                                current_hour_index = i
                                _LOGGER.debug(f"First forecast entry: {dt_local} (index {i})")
                            
                            # Check if this hour matches current or future hour
                            if dt_local_hour >= current_hour:
                                current_hour_index = i
                                first_forecast_time = dt_local
                                _LOGGER.debug(f"Found matching hour: {dt_local} (index {i}) - local hour {dt_local_hour} >= current hour {current_hour}")
                                break
                ...
                while len(forecast_24h) < 24 and len(forecast) > 0:
                    forecast_24h.append(forecast[-1])  # Repeat last forecast entry
                
                _LOGGER.debug(f"Using forecast starting from index {current_hour_index}")
                ...
                for i, hour in enumerate(forecast_24h):
                    if isinstance(hour, dict):
                        _LOGGER.debug(f"Hour {i+1}: {hour.get('datetime', 'unknown')} - Temp: {hour.get('temperature')}°")

                # Get current temperature and colors from call data
                max_color = parse_color_input(call.data.get("max_color"), 0xFF0000)  # Default red
                min_color = parse_color_input(call.data.get("min_color"), 0x0000FF)  # Default blue
                ...
                _LOGGER.debug(f"Color settings - Max: 0x{max_color:06X}, Min: 0x{min_color:06X}")
                ...
                forecast_to_use = forecast_24h[:23] if len(forecast_24h) >= 23 else forecast_24h
                ...
                if current_temp is not None:
                    ...
                    if forecast_temps:
                        ...
                        temps.append(20)
                        _LOGGER.warning("No current or forecast temperature available, using default 20°")
                ...
                while len(temps) < 24:
                    temps.append(temps[-1] if temps else 20)  # Repeat last temp or use default
                ...
                if user_min_value is not None and user_max_value is not None:
                    ...
                    _LOGGER.debug(f"Using user-defined gradient range for colors: {gradient_min}° to {gradient_max}°")
                else:
                    ...
                    _LOGGER.debug(f"Using actual forecast range for colors: {gradient_min}° to {gradient_max}°")
                ...
                _LOGGER.debug(f"  Actual 24h range: {actual_min_temp}° to {actual_max_temp}° (current + 23 forecast)")
                _LOGGER.debug(f"  Clock min/max: {clock_min_temp}° to {clock_max_temp}° (sent to device)")
                _LOGGER.debug(f"  Gradient range: {gradient_min}° to {gradient_max}° (for color calculation)")
                ...
                if first_forecast_time:
                    original_forecast_timestamp = int(first_forecast_time.timestamp())
                    _LOGGER.debug(f"Original forecast time: {first_forecast_time} (timestamp: {original_forecast_timestamp})")
                
                _LOGGER.debug(f"Using current time as forecast start (converted to local timezone):")
                ...
                _LOGGER.debug(f"  Local time: {datetime.datetime.fromtimestamp(utc_timestamp)}")
                try:
                    ...
                    # Fallback for older Python versions
                    _LOGGER.debug(f"  UTC time: {datetime.datetime.utcfromtimestamp(forecast_start_timestamp)}")
                ...
                success = await notify_service.async_send_forecast(
                    ...
                    template=call.data.get("template", None)  # Let notify service handle default template
                )
